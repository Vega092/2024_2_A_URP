...
public class MeshTrail : MonoBehaviour
{
    ...
    public Transform positionToSpawn;           // 잔상이 생성될위치

    [Header("Shader Releted")]                  // 쉐이더 관련 설정
    public Material mat;                        // 잔상에 적용될 재질
    public string shaderVarRef;                 // 쉐이더에서 사용할 변수 이름 (Alpha)
    public float shaderVarRate = 0.1f;          // 쉐이더 효과의 변화 속도
    public float shaderVarRafreshRate = 0.5f;   // 쉐이더 효과가 업데이트 되는 시간 간격

    private SkinnedMeshRenderer[] skinnedRenderer;        // 캐릭터의 3D 모델을 랜더링 하는 컴포넌트들
    private bool isTrailActive;                         // 현재 잔상 효곽가 활성화되어 있는지 확인하는 변수

    private float normalSpeed;                           // 원래 이동 속도를 저장하는 변수
    private float normalAnimSpeed;                      // 원래 애니메이션 속고를 저장하는 변수
    ...
    private void Update()
    {
        if(Input.GetKeyDown(KeyCode.Space) && !isTrailActive)           // 스페이스바
        {
            isTrailActive = true;
